...
final class CategoryFilterViewController: UIViewController, CoordinatedContent {
    typealias ContentModel = SearchFiltersContentModel

    private let filterView = CategoryFilterView()
    private var viewModel: CategoryFilterViewModel

    init(query: SearchQuery) {
        viewModel = CategoryFilterViewModel(query: query)
    ...
    @objc private func textChanged() {
        ...
        post(SearchFilterEvents.CategoryDidUpdate(category: trimmed.isEmpty ? nil : trimmed))
    }
    ...
    func update(for model: SearchFiltersContentModel) -> CoordinatedContentUpdate? {
        viewModel = CategoryFilterViewModel(query: model.query)
        return { [weak self] in
